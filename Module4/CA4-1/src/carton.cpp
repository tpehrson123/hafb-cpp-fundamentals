...
/**
 * @brief Compares two Cartons and returns true if the volume of one
 *  is within .1 of the volume of the other.
 *
 * @param rhs - the Carton on the right side of the == operator
 * @return true - returned when volumes are within .1 of each other
 * @return false - returned when volumes are more than .1 different
 */
bool Carton::operator==(const Carton& rhs) const {
  if (fabs(Volume() - rhs.Volume()) < .1) {
    return true;
  }
  return false;
}
