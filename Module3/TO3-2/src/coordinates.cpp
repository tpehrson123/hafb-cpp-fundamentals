...
{
    if(longitude_ == c2.longitude_ && latitude_ == c2.latitude_)
    {
        return false;
    }
    else
...
std::ostream &operator<<(std::ostream &os, const Coordinates &location)
{
    ...
    os << "lat_: " << location.latitude() << " long_: " << location.longitude();
    return os;
}
...
std::string Coordinates::show_gps()
{
    ...
    //                                    -             - : South and West 
    //                                    -             + : South and East
    gps_format = to_string(abs(degrees)) + "\370" 
    + to_string(minutes) + "'" 
    + to_string(seconds) + "\"";
    if(degrees >= 0)
    {
    ...
    // cout << "longitude " << longitude_ << endl;
